...
import nikmax.mtree.gallery.core.domain.models.Filters
import nikmax.mtree.gallery.core.domain.models.GalleryMode
import nikmax.mtree.gallery.core.domain.models.MediaItemDomain
import nikmax.mtree.gallery.core.domain.models.Sort
import nikmax.mtree.gallery.core.domain.usecases.ItemsUtils.Filtering.applyFilters
import nikmax.mtree.gallery.core.domain.usecases.ItemsUtils.PlainMode.getAlbumOwnFiles
import nikmax.mtree.gallery.core.domain.usecases.ItemsUtils.PlainMode.getAllNotEmptyAlbums
import nikmax.mtree.gallery.core.domain.usecases.ItemsUtils.PlainMode.plainSearchByPath
import nikmax.mtree.gallery.core.domain.usecases.ItemsUtils.Sorting.applySorting
import nikmax.mtree.gallery.core.domain.usecases.ItemsUtils.TreeMode.getItemsForPath
import nikmax.mtree.gallery.core.domain.usecases.ItemsUtils.TreeMode.getTreeSearchResult
...
class CreateItemsListToDisplayUcImpl() : CreateItemsListToDisplayUc {
...
internal object ItemsUtils {
    ...
    object PlainMode {
        fun List<MediaItemDomain.Album>.getAllNotEmptyAlbums(): List<MediaItemDomain.Album> {
            return this.filterNot { it.files.isEmpty() }
        }
    ...
    object TreeMode {
        ...
        fun List<MediaItemDomain.Album>.getTreeSearchResult(
            ...
            val baseDirectoryFilesContainsQuery = this
                ...
                ?.let { baseDirectory ->
                    baseDirectory.files.filter { it.name.contains(query, ignoreCase = true) }
                } ?: emptyList()
            val nestedAlbumsContainsQuery = this
                ...
                .let { albumsToSearch ->
                    ...
                    albumsToSearch.forEach { album ->
                        ...
                        if (album.files.any { it.name.contains(query, ignoreCase = true) }) {
